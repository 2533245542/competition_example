...
import math
from autoTuneAndRun import autoTuneAndRun
...
#%% todo clip the more recent data
# start
...
hyperparameters, feature_indices, in_batch_size, steps_ahead, dropout, recurrent_dropout, num_cells, learning_rate =((0, 2), 10, 2, 0.4, 0.3, 20, 0.004552290686991764), (0, 2), 10, 2, 0.4, 0.3, 20, 0.004552290686991764
# todo load
# start
...
for i in iteration_list:
    # todo save and load i, list_predict_future, list_actual_future, list_loss_future, testDataTimeStamps
    # start
    ...
    if i == (sliding_window_size - 1) or (sliding_window_size - 1 - i) % tune_frequency == 0:
        hyperparameters = autoTuneAndRun(data[:train_length])
    ...
    loss_past = np.abs(y_predict_past_1D - y_actual_past_1D).mean()
    ...
    loss_future = np.abs(y_future_predict_1D - y_future_actual_1D).mean()
    ...
    # plt.show()
    plt.savefig('plots/{}_diagnosis_plot_{}.png'.format(site, i), dpi=400)
...
#     list_actual_future = []
#     list_loss_future = []
#     use the saved dataframe, find the last sliding_window_size number of dates
# start
...
'''
cp -r caseOnly_resumable caseOnly_resumable100
cp -r caseOnly_resumable caseOnly_resumable200
cp -r caseOnly_resumable caseOnly_resumable250
cp -r caseOnly_resumable caseOnly_resumable300


vim caseOnly_resumable100/config.py
vim caseOnly_resumable200/config.py
vim caseOnly_resumable250/config.py
vim caseOnly_resumable300/config.py
...
cd caseOnly_resumable100; python3 case_model_auto_tuned_resumable.py
cd caseOnly_resumable200; python3 case_model_auto_tuned_resumable.py
cd caseOnly_resumable250; python3 case_model_auto_tuned_resumable.py
cd caseOnly_resumable300; python3 case_model_auto_tuned_resumable.py

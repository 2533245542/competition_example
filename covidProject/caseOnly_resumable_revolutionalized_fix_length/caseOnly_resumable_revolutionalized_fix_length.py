...
from config import sliding_window_size, num_epochs, tune_frequency, out_batch_size, days_of_trace_back_for_features, target_column_index, resumable_file, total_prediction_loss_timestamp_record, train_loss, data_length, calculate_loss_future
...
def case_model_auto_tuned():
    ...
    # dfSite = dfSite[-data_length:]  # todo remove this
    ...
    for i in iteration_list:
        ...
        if i == (sliding_window_size - 1) or (sliding_window_size - 1 - i) % tune_frequency == 0:
            ...
            hyperparameters = autoTuneAndRun.autoTuneAndRun(data[:train_length])
        ...
        print('completed {}/{} sliding window, current MLSE {}, MLSE so far {}'.format(sliding_window_size - i, sliding_window_size, calculate_loss_future(y_true=list_actual_future[-1:], y_pred=list_predict_future[-1:]), calculate_loss_future(y_true=list_actual_future, y_pred=list_predict_future)), 'Hyperparamters using:', hyperparameters)
    ...
    print('MLSE_n_day_avg_window_size_{}'.format(sliding_window_size), sum(list_loss_future) / sliding_window_size)

    plt.clf()
    plt.cla()
    plt.close()
    pd.DataFrame({'actual number of phone calls related to COVID-19 symtoms': list_actual_future, 'predicted number of phone calls related to COVID-related symtoms': list_predict_future}, index=testDataTimeStamps).plot(title='site {}, MLSE={}'.format(site, calculate_loss_future(y_true=list_actual_future, y_pred=list_predict_future)))
    # plt.show()

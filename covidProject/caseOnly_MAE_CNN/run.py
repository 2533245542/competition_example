...
from keras.layers import Dense, TimeDistributed, Dropout, MaxPooling1D, Conv1D, Flatten
...
import math
import tuneWorker
from config import sliding_window_size, num_epochs, tune_frequency, out_batch_size, days_of_trace_back_for_features, target_column_index, resumable_file, total_prediction_loss_timestamp_record, train_loss, data_length, calculate_loss_future, clipLeft, clipRight
...
def run():
    ...
    dfSite = dfSite[clipLeft: clipRight]
    ...
    for i in iteration_list:
        ...
        dfSite_tmp = dfSite_tmp.iloc[len(dfSite_tmp) - i - data_length :len(dfSite_tmp) - i, :] # todo, change this
        ...
        if i == (sliding_window_size - 1) or (sliding_window_size - 1 - i) % tune_frequency == 0:
            ...
            hyperparameters = tuneWorker.doTune(data[:train_length])
        ...
        y_actual_past_1D = pd.Series(y_actual_past.reshape(-1)).apply(lambda x: x if x > 0 else 0) # when transfomred back, some 0 might become -0.0000001
        ...
        loss_past = metrics.calculate_loss_future(y_true=y_actual_past_1D, y_pred=y_predict_past_1D)
        ...
        # plt.show()
        plt.savefig('plots/{}_diagnosis_plot_{}.png'.format(site, sliding_window_size - i), dpi=200)
        print('completed {}/{} sliding window, current MAE {}, MAE so far {}'.format(sliding_window_size - i, sliding_window_size, calculate_loss_future(y_true=list_actual_future[-1:], y_pred=list_predict_future[-1:]), calculate_loss_future(y_true=list_actual_future, y_pred=list_predict_future)), 'Hyperparamters using:', hyperparameters)

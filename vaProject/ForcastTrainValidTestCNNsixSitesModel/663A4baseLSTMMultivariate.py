...
n_steps_in, n_steps_out = 4, 1  # n_steps_out means the number of future steps. 1 means predicting one future
train_ratio = 0.75

test_n_day_length = 18
...
model = Sequential()
model.add(
    TimeDistributed(Conv1D(filters=50, kernel_size=1, activation='relu'), input_shape=(None, n_steps, n_features)))
model.add(TimeDistributed(MaxPooling1D(pool_size=2)))
model.add(TimeDistributed(Flatten()))
model.add(LSTM(50, activation='relu', dropout=0.8, recurrent_dropout=0))
model.add(Dense(1))

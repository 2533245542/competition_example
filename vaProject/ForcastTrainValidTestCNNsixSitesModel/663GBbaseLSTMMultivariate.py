...
model = Sequential()
model.add(
    TimeDistributed(Conv1D(filters=150, kernel_size=1, activation='relu'), input_shape=(None, n_steps, n_features)))
model.add(TimeDistributed(MaxPooling1D(pool_size=4)))
model.add(Dropout(0.0))
model.add(TimeDistributed(Flatten()))
model.add(LSTM(55, activation='relu', dropout=0.55, recurrent_dropout=0.2))
model.add(Dense(1))

model.compile(optimizer=Adam(lr=0.00015), loss='mae')
...
callbacks_list = [
    ...
    keras.callbacks.ModelCheckpoint(filepath=checkPointFilePath, monitor='val_loss', save_best_only=True,
                                    verbose=0)]
# history = model.fit(X_train, y_train, validation_data=(X_test, y_test), epochs=50, verbose=0, callbacks=callbacks_list)
...
# # plt.show()
#
# # print(np.abs(pd.Series([2] * y_test.shape[0]) - y_test).mean())  # 1.87
#
# %% test future n days

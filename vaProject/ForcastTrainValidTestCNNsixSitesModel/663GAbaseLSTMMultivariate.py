...
n_steps_in, n_steps_out = 81, 1  # n_steps_out means the number of future steps. 1 means predicting one future
train_ratio = 1

test_n_day_length = 95
...
model = Sequential()
model.add(
    TimeDistributed(Conv1D(filters=50, kernel_size=5, activation='relu'), input_shape=(None, n_steps, n_features)))
model.add(TimeDistributed(MaxPooling1D(pool_size=1)))
model.add(TimeDistributed(Flatten()))
model.add(LSTM(50, activation='relu', dropout=0.55, recurrent_dropout=0.4))
model.add(Dense(1))
...
callbacks_list = [
    ...
    keras.callbacks.ModelCheckpoint(filepath=checkPointFilePath, monitor='val_loss', save_best_only=True,
                                    save_weights_only=True,
                                    verbose=0)]
# history = model.fit(X_train, y_train, validation_data=(X_test, y_test), epochs=100, verbose=0, callbacks=callbacks_list)
...
# plt.show()
#
# # np.abs(pd.Series([1] * y_test.shape[0]) - y_test).mean()
#
# %% test future n days

...
# todo anchorTechniqueName =
anchorValue = 10
# todo techniquesNames = []
hyperparameterValues = [5, 7, 10, 20, 30, 40, 50, 60, 70]
ax1_ylim = [-0.5, 0.5]
ax2_ylim = [-0.5, 0.5]
x_axis = file.split('.')[0]  # x_axis = files[0].split('.')[0]
...
# todo read a list of files, merge all of them to be df, assign techniqueName for each of them
df = pd.read_csv(file)
...
# todo apply this filter  to all dfS
...
# todo concate df's together and do this for all
print(df.hyperparameterName.unique())
print(df.testError.isna().groupby([df.hyperparameterValue]).sum().astype(int).reset_index(name='count'))  # todo print(df.testError.isna().groupby([df.techniqueName]).sum().astype(int).reset_index(name='count'))
print('not done:', df.testError.isna().groupby([df.hyperparameterValue]).sum().astype(int).reset_index(name='count').sum().values[1])  # todo print('not done:', df.testError.isna().groupby([df.techniqueName]).sum().astype(int).reset_index(name='count').sum().values[1])
print(df.testError.isna().groupby([df.hyperparameterValue, df.dataset]).sum().astype(int).reset_index(name='count')) # todo print(df.testError.isna().groupby([df.techniqueName, df.dataset]).sum().astype(int).reset_index(name='count'))
...
for value in hyperparameterValues:  # todo for techniqueName in techniqueNames:
    # todo change to  anchorDf = df[lambda df: df.techniqueName == anchorTechniqueName][['dataset', 'testError', 'time']].groupby('dataset', as_index=False).mean()
    anchorDf = df[lambda df: df.hyperparameterValue == anchorValue][['dataset', 'testError', 'time']].groupby('dataset', as_index=False).mean()
    # todo change to  nonAnchorDf = df[lambda df: df.techniqueName == techniqueName][['dataset', 'testError', 'time']].groupby('dataset', as_index=False).mean()
...
# todo change to print('technique names', techniqueNames)
...
for value, merge_df in zip(hyperparameterValues, merge_df_list):  # todo for techniqueName, merge_df in zip(techniqueNames, merge_df_list)
    merge_df['hyperparameterValue'] = value  # todo merge_df['techniqueName'] = techniqueName
...
summaryDf = pd.DataFrame(
    # todo {'technique names': techniqueNames,
...
#%%% make plots
# todo errorRateValueDf = pd.DataFrame({x_axis: techniqueNames, 'overall change value': errorChangeValueList, 'metric': ['overall error rate change value'] * len(techniqueNames)})
# todo errorRateRatioDf = pd.DataFrame({x_axis: techniqueNames, 'overall change ratio': errorChangeRatioList, 'metric': ['overall error rate change ratio'] * len(techniqueNames)})
# todo timeValueDf = pd.DataFrame({x_axis: techniqueNames, 'overall change value': timeChangeValueList, 'metric': ['overall time change value'] * len(techniqueNames)})
# todo timeRatioDf = pd.DataFrame({x_axis: techniqueNames, 'overall change ratio': timeChangeRatioList, 'metric': ['overall time change ratio'] * len(techniqueNames)})
